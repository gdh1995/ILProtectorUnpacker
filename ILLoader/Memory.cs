...
namespace ILLoader
{
    internal static class Memory
    {
        ...
        internal static long GdhReplace(MethodInfo oldMethod, MethodInfo newMethod)
        {
            var addressSrc = GetAddress(oldMethod);
            var addressDst = GetAddress(newMethod);
            var oldHandleValue = oldMethod.MethodHandle.Value;
            var newHandleValue = newMethod.MethodHandle.Value;
            unsafe
            {
                if (oldMethod.IsVirtual)
                {
                    ulong* methodDesc = (ulong*)oldHandleValue.ToPointer();
                    int index = (int)(((*methodDesc) >> 32) & 0xFF);

                    long* classStart = (long*)oldMethod.DeclaringType.TypeHandle.Value.ToPointer();
                    classStart += 8;
                    classStart = (long*)*classStart;
                    long* tar = classStart + index;
                    if (*tar != addressSrc.ToInt64() && *tar != addressDst.ToInt64())
                        throw new InvalidOperationException("unknown structure of the old virtual method");
                    *tar = addressDst.ToInt64();
                }
                else
                {
                    long* old = (long*)oldHandleValue.ToPointer() + 1;
                    if (*old != addressSrc.ToInt64() && *old != addressDst.ToInt64())
                        throw new InvalidOperationException("unknown structure of the old method");
                    *old = addressDst.ToInt64();

                    long* newPtr = (long*)newHandleValue.ToPointer() + 1;
                    if (*newPtr != addressSrc.ToInt64() && *newPtr != addressDst.ToInt64())
                        throw new InvalidOperationException("unknown structure of the new method");
                    *newPtr = addressSrc.ToInt64();
                }
            }
            return addressSrc.ToInt64();
        }
    }
}